...
| R-LIB-03 | `npm test` fails if any item violates the contract (`src/library/validate.ts`): missing fields, duplicate ids, unknown sockets or tags, build error, part with > 6 000 triangles, item with > 24 meshes, non-role material. |
...
| ID | Requirement |
|---|---|
| R-GEN-01 | **Name generator**: deterministic given a seed; takes `sex` and `powerSet.category` as inputs; ≥ 5 distinct patterns; ≥ 60 000 possible outputs overall; never produces the same name twice in 20 consecutive rolls with different seeds (tested). Names are editable and lockable. |
...
| R-SAVE-03 | **Share URL**: full character encoded in the URL hash (`#c=<base64url>`), ≤ 400 characters for any valid character. Opening the URL loads exactly that character. Unknown item ids degrade to `null` (or undersuit) with a toast, never a crash. |
