...
export function buildBody(sex: Sex): ItemBuild {
  ...
  const jaw = kit.at(kit.scaled(kit.mesh(kit.sphere(r.headRadius * 0.82, 14), skin), 1, 0.9, 0.95), 0, -r.headRadius * 0.42, r.headRadius * 0.08);
  const eyeL = kit.at(kit.mesh(kit.sphere(r.headRadius * 0.09, 8), 'dark'), r.headRadius * 0.34, r.headRadius * 0.1, r.headRadius * 0.9);
  const eyeR = kit.at(kit.mesh(kit.sphere(r.headRadius * 0.09, 8), 'dark'), -r.headRadius * 0.34, r.headRadius * 0.1, r.headRadius * 0.9);
  const headGroup = kit.group(head, jaw, eyeL, eyeR);

  const neck = kit.at(kit.mesh(kit.cyl(r.neckRadius * 0.95, r.neckRadius, r.neckLen * 1.6, 12), skin), 0, r.neckLen * 0.55, 0);

  const cw = r.chestHalfW * 2, cd = r.chestDepth, cl = r.chestLen;
  const chest = kit.at(kit.scaled(kit.mesh(kit.sphere(1, 18), skin), cw * 0.5, cl * 0.36, cd * 0.5), 0, cl * 0.18, 0);
  const upper = kit.at(kit.mesh(kit.rbox(cw * 0.96, cl * 0.5, cd * 0.96, cd * 0.3), skin), 0, cl * 0.22, 0);
  const abdomen = kit.at(kit.scaled(kit.mesh(kit.sphere(1, 16), skin), r.waistHalfW * (f ? 0.88 : 1), cl * 0.36, r.waistDepth * 0.5), 0, -cl * 0.22, 0);
  ...
  const pelvis = kit.at(kit.scaled(kit.mesh(kit.sphere(1, 16), skin), r.pelvisHalfW * (f ? 1.0 : 0.98), r.hu * 0.36, r.pelvisDepth * 0.5), 0, -r.hu * 0.02, 0);
  const pelvisBox = kit.at(kit.mesh(kit.rbox(r.pelvisHalfW * 1.9, r.hu * 0.5, r.pelvisDepth * 0.96, r.pelvisDepth * 0.3), skin), 0, r.hu * 0.08, 0);
  ...
  const hand = () => kit.at(kit.mesh(kit.rbox(r.handSize * 0.5, r.handLen * 0.95, r.handSize * 0.3, r.handSize * 0.08), skin), 0, r.handLen * 0.48, 0);
  ...
  const foot = () => kit.at(kit.mesh(kit.rbox(r.footW, r.footH * 0.9, r.footLen, r.footH * 0.2), skin), 0, -r.footH * 0.5, r.footLen * 0.28);
