...
export function PalettePanel() {
  ...
  return (
    <section
      className={`bottom-right panel${collapsed ? ' collapsed' : ''}`}
      aria-label="Colours"
    ...
    >
      <div className="panel-head">
        ...
        <button
          ...
          aria-label={locked ? 'Unlock colours' : 'Lock colours'}
          aria-pressed={locked}
          ...
          {ROLES.map(([role, label]) => (
            <label key={role} className="role-swatch">
              ...
              <input
                ...
                onInput={(e) => setPalette({ [role]: (e.target as HTMLInputElement).value }, false)}
                onChange={(e) => setPalette({ [role]: e.target.value }, true)}
              />
            </label>
          ))}
        </div>
        <div className="presets" role="group" aria-label="Palette presets">
          {PALETTE_PRESETS.map((p) => (
            <button
              ...
              onClick={() => setPalette({ primary: p.primary, secondary: p.secondary, accent: p.accent })}
            >
