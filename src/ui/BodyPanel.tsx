...
export function BodyPanel() {
  ...
  return (
    <section
      className={`bottom-left panel${collapsed ? ' collapsed' : ''}`}
      aria-label="Body"
      data-testid="body-panel"
    >
      <div className="panel-head">
        ...
        <button
          ...
          aria-label={locked ? 'Unlock body' : 'Lock body'}
          aria-pressed={locked}
      ...
      <div className="panel-body">
          ...
          <div className="swatches" role="group" aria-label="Skin tone">
            {SKIN_TONES.slice(0, 9).map((t) => (
              <button
                ...
                aria-label={t.name}
                aria-pressed={body.skinTone === t.hex}
                title={t.name}
                onClick={() => updateBody({ skinTone: t.hex }, true)}
              />
            ))}
            <span className="swatch custom" title="Custom skin tone">
              <input
                ...
                onInput={(e) => updateBody({ skinTone: (e.target as HTMLInputElement).value }, false)}
                onChange={commit}
              />
            </span>
          </div>
        ...
        <div className="slider">
          ...
          <input
            ...
            value={body.height}
            aria-label="Height"
            style={{ '--fill': `${body.height * 100}%` } as never}
            onChange={(e) => updateBody({ height: Number(e.target.value) }, false)}
            onPointerUp={commit}
            onKeyUp={commit}
            onBlur={commit}
          />
        ...
        <div className="slider">
          ...
          <input
            ...
            max={1}
            step={0.01}
            value={body.musculature}
            aria-label="Musculature"
            style={{ '--fill': `${body.musculature * 100}%` } as never}
            onChange={(e) => updateBody({ musculature: Number(e.target.value) }, false)}
            onPointerUp={commit}
            onKeyUp={commit}
            onBlur={commit}
          />
